...
class Client():
    ...
    def __make_api_call(self, path, method='GET', files=None, data=None, raw=False):
        ...
        # print("Sending: (method:{}, apicall_path:{}, auth={}, headers={}, files={}, data={})".format(method, apicall_path, self.__credentials, self.__headers, files, data))
        response = requests.request(method, apicall_path, auth=self.__credentials, headers=self.__headers, files=files, data=data)
       
        # print("f(x) malpedia_client: __make_api_call: {}".format(response))
